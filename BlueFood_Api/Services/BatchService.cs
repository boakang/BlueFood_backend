...
DECLARE @PrevHash VARBINARY(32) = (SELECT TOP(1) ThisHash FROM audit.AuditLogs WITH (UPDLOCK, HOLDLOCK) ORDER BY AuditId DESC);
DECLARE @Payload NVARCHAR(MAX) = CONCAT(N'BatchCode=', @BatchCode, N';CertificateId=', @CertificateId);
DECLARE @HashInput NVARCHAR(MAX) = CONCAT(ISNULL(CONVERT(NVARCHAR(64), @PrevHash, 2), N''), N'|BATCH_CERT|', CONVERT(NVARCHAR(36), @BatchId), N'|ATTACH_CERT|', @Actor, N'|', CONVERT(NVARCHAR(33), @Now, 126), N'|', @Payload);
DECLARE @ThisHash VARBINARY(32) = HASHBYTES('SHA2_256', @HashInput);

INSERT INTO audit.AuditLogs(EntityName, EntityId, ActionType, ActionAt, Actor, PayloadText, PrevHash, ThisHash)
VALUES(N'BATCH_CERT', CONVERT(NVARCHAR(36), @BatchId), N'ATTACH_CERT', @Now, @Actor, @Payload, @PrevHash, @ThisHash);
